...
Converts markdown OCR output + layout analysis to formatted .docx files.
Combines text content from OCR with visual formatting from layout analysis.
"""
...
def _docx_alignment(name: str) -> int:
    """Convert alignment name to python-docx constant."""
    if name == "center":
...
def _join_wrapped_lines(lines: List[str]) -> str:
    """Join wrapped lines, respecting hyphenation."""
    out = ""
    for ln in lines:
        if not out:
            out = ln
...
def _majority_bool(values: List[bool]) -> bool:
    """Return True if majority of values are True."""
    if not values:
        return False
...
class MarkdownInline:
    """Inline markdown element with formatting."""
    text: str
    bold: bool = False
    italic: bool = False
    code: bool = False
...
def _match_text_lines_to_layout_lines(text_lines: List[str], layout: LayoutAnalysis) -> List[Optional[int]]:
    """Match non-empty text lines to layout line indices."""
...
def _line_index_to_layout_index(text_lines: List[str], layout: LayoutAnalysis) -> List[Optional[int]]:
    """Create mapping from text line index to layout line index."""
...
def _styles_from_layout(
    ...
    """Extract majority formatting from layout for given text lines."""
    aligns: List[str] = []
    bolds: List[bool] = []
    italics: List[bool] = []
...
def _add_inline_runs(paragraph, text: str, default_bold: bool, default_italic: bool) -> None:
    """Add text runs to paragraph with inline markdown formatting."""
    # Don't apply defaults if text already has markdown
...
def build_docx(title: str, ocr_text: str, layout: LayoutAnalysis) -> bytes:
    """
    Build .docx file from OCR text + layout analysis.
    
    Args:
        title: Document title (can be empty)
        ocr_text: Markdown-formatted text from OCR
        layout: Layout analysis with line positions and styles
    ...
    # Normalize text and parse markdown
    ...
    for block in blocks:
        # Get formatting from layout
        align, default_bold, default_italic = _styles_from_layout(
            block.source_line_idxs, text_line_to_layout, layout
        )
